...
export default function SuperAdminDashboard() {
  const [stats, setStats] = React.useState([
    ...
    },
    {
      ...
      value: '0',
      helperText: 'Require action',
      trend: { positive: false, label: '--' },
      icon: '⏳',
      accent: '#FF7A00',
    },
    {
  ...
  React.useEffect(() => {
    ...
    setStats([
      ...
      },
      {
        ...
        trend: {
          positive: true,
          label: `${((conversions / (totalQuotes || 1)) * 100).toFixed(1)}% Rate`,
        },
        icon: '✅',
        accent: '#1ABC9C',
      },
  ...
  const auditLogs = [
    {
      ...
      user: 'Dealer Manager A',
      timestamp: '5 mins ago',
      status: 'Success',
      detail: 'New quote #1055 created',
    },
    {
      id: 2,
      action: 'Price Updated',
      user: 'Dealer Manager B',
      timestamp: '20 mins ago',
      status: 'Success',
      detail: 'Updated price for Quote #1042',
    },
    {
      id: 3,
      action: 'Dealership Pending',
      user: 'System',
      timestamp: '1 hour ago',
      status: 'Warning',
      detail: 'New registration request: "City Motors"',
    },
    {
      id: 4,
      action: 'User Login',
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-4">
            <Link
              href="/dealerships?action=add"
              className="flex items-center gap-2 bg-[rgb(var(--color-primary))] text-white px-5 py-3 rounded-xl hover:bg-[rgb(var(--color-primary-dark))] transition-all shadow-lg shadow-[rgb(var(--color-primary)/0.3)] font-medium"
            >
              <Plus size={20} />
              Add New Dealership
            </Link>
            <Link
              ...
              className="flex items-center gap-2 bg-[rgb(var(--color-surface))] text-[rgb(var(--color-text))] border border-[rgb(var(--color-border))] px-5 py-3 rounded-xl hover:bg-[rgb(var(--color-background))] transition-all font-medium"
            >
