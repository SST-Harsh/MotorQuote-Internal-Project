...
export default function ApprovalsView() {
  ...
  useEffect(() => {
    ...
    const syncData = (storedList, currentList, type) => {
      ...
      const handledIds = new Set(
        storedList.filter((item) => item.status?.toLowerCase() !== 'pending').map((item) => item.id)
      );
  ...
  const TabButton = ({ id, label, count, icon: Icon }) => (
    ...
    >
      ...
      {label} ({count})
      {activeTab === id && (
        <div className="absolute bottom-0 left-0 w-full h-0.5 bg-[rgb(var(--color-primary))] rounded-t-full"></div>
      )}
    </button>
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[rgb(var(--color-text))] text-sm">
          Manage pending approvals for all entities
        </p>
      </div>
      ...
      <div className="flex border-b border-[rgb(var(--color-border))] overflow-x-auto scrollbar-hide">
        ...
        <TabButton
          id="dealerships"
          label="Dealerships"
          count={pendingDealerships.length}
          icon={Building2}
        />
            ...
            renderItem={(item) => (
              <div>
                ...
                <div className="text-sm text-[rgb(var(--color-text))] flex flex-wrap gap-x-4 gap-y-1 mt-1">
                  <span>
                    Dealer:{' '}
                    <span className="font-medium text-[rgb(var(--color-text))]">{item.dealer}</span>
                  </span>
                  <span>
                    Amount:{' '}
                    <span className="font-medium text-[rgb(var(--color-text))]">{item.amount}</span>
                  </span>
  ...
  return (
    <div className="divide-y divide-[rgb(var(--color-border))]">
      {items.map((item) => (
        <div
          key={item.id}
          className="p-4 flex items-start sm:items-center justify-between gap-4 hover:bg-[rgb(var(--color-background))] transition-colors"
        >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 text-[rgb(var(--color-error))] hover:bg-[rgb(var(--color-error))/0.1] rounded-lg transition-colors"
              title="Reject"
            >
              <X size={20} />
            </button>
            <button
              ...
              className="p-2 text-[rgb(var(--color-success))] hover:bg-[rgb(var(--color-success))/0.1] rounded-lg transition-colors"
              title="Approve"
            >
              <Check size={20} />
            </button>
