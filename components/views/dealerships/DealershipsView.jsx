...
export default function DealershipsView() {
  ...
  const handleSaveDealership = (dealerData) => {
    ...
    if (dealerData.id && dealerships.some((d) => d.id === dealerData.id)) {
      setDealerships((prev) =>
        prev.map((d) => (d.id === dealerData.id ? { ...d, ...dataWithFiles } : d))
      );
      Swal.fire({
        icon: 'success',
        title: 'Updated!',
        text: `${dealerData.name} has been updated successfully.`,
        timer: 2000,
      ...
      Swal.fire({
        icon: 'success',
        title: 'Added!',
        text: `${newDealer.name} has been registered successfully.`,
        timer: 2000,
        showConfirmButton: false,
      });
    }
  ...
  const handleBulkAction = (action) => {
    if (action === 'delete') {
      ...
      setDealerships((prev) =>
        prev.map((d) =>
          selectedIds.includes(d.id) ? { ...d, status: 'suspended', statusLabel: 'Suspended' } : d
        )
      );
      setSelectedIds([]);
      ...
      setDealerships((prev) =>
        prev.map((d) =>
          selectedIds.includes(d.id) ? { ...d, status: 'active', statusLabel: 'Active' } : d
        )
      );
      setSelectedIds([]);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-[rgb(var(--color-text))] text-sm">
            Manage all your dealership partners
          </p>
        </div>
        <Button
          onClick={() => {
        ...
        columns={[
          ...
          },
          {
            ...
            accessor: (item) => {
              ...
              return (
                <span
                  className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${statusStyles[item.status?.toLowerCase()] || statusStyles.pending}`}
                >
                  {item.statusLabel ||
                    (item.status
