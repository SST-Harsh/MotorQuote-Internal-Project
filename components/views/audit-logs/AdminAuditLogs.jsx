...
const getMethodColor = (method) => {
  switch (method?.toUpperCase()) {
    case 'GET':
      return 'text-[rgb(var(--color-info))] bg-[rgb(var(--color-info))]/10 border-[rgb(var(--color-info))]/20';
    case 'POST':
      return 'text-[rgb(var(--color-success))] bg-[rgb(var(--color-success))]/10 border-[rgb(var(--color-success))]/20';
    case 'PUT':
      return 'text-[rgb(var(--color-warning))] bg-[rgb(var(--color-warning))]/10 border-[rgb(var(--color-warning))]/20';
    case 'DELETE':
      return 'text-[rgb(var(--color-error))] bg-[rgb(var(--color-error))]/10 border-[rgb(var(--color-error))]/20';
    default:
...
const getSeverityBadge = (level) => {
  const styles = {
    critical:
      'bg-[rgb(var(--color-error))]/10 text-[rgb(var(--color-error))] border-[rgb(var(--color-error))]/20 ring-[rgb(var(--color-error))]/10',
    error:
      'bg-[rgb(var(--color-error))]/10 text-[rgb(var(--color-error))] border-[rgb(var(--color-error))]/20 ring-[rgb(var(--color-error))]/10',
    warning:
      'bg-[rgb(var(--color-warning))]/10 text-[rgb(var(--color-warning))] border-[rgb(var(--color-warning))]/20 ring-[rgb(var(--color-warning))]/10',
    info: 'bg-[rgb(var(--color-info))]/10 text-[rgb(var(--color-info))] border-[rgb(var(--color-info))]/20 ring-[rgb(var(--color-info))]/10',
  };
...
export default function AdminAuditLogs() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Pagination might be at root for direct array or in data.pagination from API wrapper or inner structure
    ...
    if (activeTab === 'audit') {
      ...
      return [
        ...
        },
        {
          ...
          accessor: (row) => (
            <span
              className={`inline-block px-2.5 py-1 rounded-full text-xs font-bold border ring-1 ${getSeverityBadge(row.severity)}`}
            >
              {row.severity}
            </span>
          ),
      ...
      return [
        ...commonCols,
        {
          ...
          accessor: (row) => (
            <div>
              <div className="font-semibold text-[rgb(var(--color-text))]">{row.action}</div>
        ...
        },
        {
          ...
          className: 'text-[rgb(var(--color-error))] italic',
        },
        {
          header: '',
          accessor: (row) => (
      ...
      <div className="space-y-3">
        ...
        {activityData.map((item, idx) => (
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`p-2 rounded-md ${item.action === 'LOGIN' ? 'bg-purple-500/10 text-purple-500' : 'bg-[rgb(var(--color-primary))/0.1] text-[rgb(var(--color-primary))]'}`}
              >
          ...
          actions={
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full mt-2 w-48 bg-[rgb(var(--color-surface))] border border-[rgb(var(--color-border))] rounded-xl shadow-xl overflow-hidden invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all z-20">
                ...
                >
                  <FileText size={14} className="text-[rgb(var(--color-info))]" /> Export as CSV
                </button>
              </div>
            </div>
          }
