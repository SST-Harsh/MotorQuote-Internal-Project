...
import Loader from "../../(ReusableDashboardComponents)/Loader";
...
import { Plus, Shield, Users, Lock, Eye, Edit2, Trash2, CheckCircle, XCircle, FileText, ChevronDown, Briefcase, DollarSign, Headphones, Wrench } from "lucide-react";
...
const initialRoles = [
    { id: 'r-1', name: 'Manager', description: 'Can manage dealerships and quotes', usersCount: 2, status: 'active', lastUpdated: 'Dec 12, 2024' },
    { id: 'r-2', name: 'Manager', description: 'Can manage dealerships and quotes', usersCount: 5, status: 'active', lastUpdated: 'Dec 15, 2024' },
    { id: 'r-3', name: 'Seller', description: 'Can create quotes and view assigned leads', usersCount: 12, status: 'active', lastUpdated: 'Dec 20, 2024' },
    { id: 'r-4', name: 'Support', description: 'Can view tickets and basic details', usersCount: 3, status: 'active', lastUpdated: 'Dec 18, 2024' },
];
...
export default function RolesPermissionsView() {
    ...
    const handleSave = (roleData) => {
        if (roleData.id && roles.some(r => r.id === roleData.id)) {
            ...
            // Create
            const newRole = { ...roleData, id: `r-${Date.now()}`, usersCount: 0, status: 'active', lastUpdated: new Date().toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }) };
    ...
    const handleDelete = (id) => {
        Swal.fire({
            title: 'Are you sure?', text: "This might affect users assigned to this role!", icon: 'warning',
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-[rgb(var(--color-text))] text-sm">Manage access levels and system capabilities</p>
                </div>
                <button
                    onClick={() => { setEditingRole(null); setIsAddModalOpen(true); }}
                    ...
                    },
                    {
                        ...
                        accessor: (role) => (
                            <div className="flex items-center justify-center gap-2">
                                <button
                                    onClick={(e) => { e.stopPropagation(); setViewingRole(role); }}
                                    className="p-1.5 text-[rgb(var(--color-text))] hover:text-[rgb(var(--color-primary))] hover:bg-[rgb(var(--color-primary)/0.1)] rounded-lg transition-colors" title="View Details">
                                    <Eye size={16} />
                                </button>
                                <button
                                    onClick={(e) => { e.stopPropagation(); setEditingRole(role); setIsAddModalOpen(true); }}
                                    className="p-1.5 text-[rgb(var(--color-text))] hover:text-[rgb(var(--color-info))] hover:bg-[rgb(var(--color-info)/0.1)] rounded-lg transition-colors" title="Edit Role">
                                    <Edit2 size={16} />
                                </button>
                                <button
                                    onClick={(e) => { e.stopPropagation(); handleDelete(role.id); }}
                                    className="p-1.5 text-[rgb(var(--color-text))] hover:text-[rgb(var(--color-error))] hover:bg-[rgb(var(--color-error)/0.1)] rounded-lg transition-colors" title="Delete Role">
                                    <Trash2 size={16} />
                                </button>
                            </div>
                        ),
            ...
            <GenericFormModal
                ...
                onClose={() => { setIsAddModalOpen(false); setEditingRole(null); }}
                onSave={handleSave}
                title={editingRole ? "Edit Role" : "Create New Role"}
                initialData={editingRole}
                validationSchema={roleSchema}
                fields={[
                    { name: 'name', label: 'Role Name', placeholder: 'e.g. Manager', icon: Shield, type: 'select', options: [{ value: 'Manager', label: 'Manager' }, { value: 'Seller', label: 'Seller' }, { value: 'Support', label: 'Support' }] },
                    { name: 'description', label: 'Description', placeholder: 'Access level description...', icon: FileText, type: 'textarea' },
                    { name: 'status', label: 'Status', placeholder: 'Select status', icon: Shield, type: 'select', options: [{ value: 'active', label: 'Active' }, { value: 'inactive', label: 'Inactive' }] },
                    { name: 'permissions', label: 'Permissions', type: 'checkbox-group', options: [{ value: 'all', label: 'All Access' }, { value: 'read', label: 'View Only' }, { value: 'write', label: 'Edit/Write' }, { value: 'delete', label: 'Delete Capabilities' }] },
                ]}
            />
            ...
            <DetailViewModal
                ...
                data={viewingRole}
                title="Role Details"
                onEdit={(r) => { setViewingRole(null); setEditingRole(r); setIsAddModalOpen(true); }}
