...
import { Bell, Trash2, Plus, Info, CheckCircle, AlertTriangle, XCircle, Users, Calendar, Edit2 } from 'lucide-react';
...
export default function NotificationsView() {
    const { userNotifications, createNotification, updateNotification, deleteNotification, isLoading } = useNotifications(); // Use userNotifications
    const { user } = useAuth();
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-[rgb(var(--color-text))] text-sm">Broadcast messages and manage system alerts</p>
                </div>
                <button
                ...
                columns={[
                        ...
                        accessor: (row) => (
                            <div className="flex items-start gap-3">
                                <div className={`p-2 rounded-lg mt-1 
                                    ${row.type === 'info' ? 'bg-blue-100 text-blue-600' :
                                        row.type === 'success' ? 'bg-green-100 text-green-600' :
                                            row.type === 'warning' ? 'bg-yellow-100 text-yellow-600' :
                    ...
                    },
                    {
                        ...
                        accessor: (row) => {
                            ...
                            if (isScheduled) {
                                return <span className="text-xs font-medium text-amber-600 bg-amber-50 px-2 py-1 rounded border border-amber-200">Scheduled</span>
                            }
                            return <span className="text-xs font-medium text-green-600 bg-green-50 px-2 py-1 rounded border border-green-200">Sent</span>
                        }
                    },
                    {
                        header: "Date",
                        accessor: (row) => {
                            ...
                            return (
                                <div className="flex flex-col">
                                    <span className="text-xs text-[rgb(var(--color-text))] font-medium">{new Date(date).toLocaleDateString()}</span>
                                    <span className="text-[10px] text-[rgb(var(--color-text-muted))]">{new Date(date).toLocaleTimeString()}</span>
                                </div>
                            )
                        },
                        sortable: true
                    ...
                    {
                        header: "Read Count",
                        accessor: (row) => <span className="text-xs font-mono bg-gray-100 px-2 py-1 rounded text-gray-600">{row.readBy?.length || 0}</span>
                    },
                    {
                        header: "",
                        accessor: (row) => {
                            const canManage = user?.role?.toLowerCase() === 'super_admin' || row.creatorId === user?.id;
            ...
            <GenericFormModal
                ...
                onClose={() => { setIsAddModalOpen(false); setEditingNotification(null); }}
                onSave={handleSave}
                ...
                fields={[
                    ...
                    { name: 'message', label: 'Message', type: 'textarea', placeholder: 'Enter the full message content...', icon: Info },
                    {
                        name: 'type',
                        label: 'Type',
                        type: 'select',
