...
import {
    Globe, Shield, Bell, Lock, Mail, Server, Palette, AlertTriangle, Save, FileText, DollarSign
} from "lucide-react";
    ...
    return (
        <div className="bg-[rgb(var(--color-surface))] rounded-xl shadow-sm border border-[rgb(var(--color-border))] p-8 space-y-8">
            ...
            <div className="p-3 bg-[rgb(var(--color-info))/0.1] rounded-lg flex items-center gap-3 text-xs text-[rgb(var(--color-info))]">
                ...
                <span>These settings affect the entire platform layout, security protocols, and communication channels.</span>
            </div>
            ...
            <div className="space-y-6 pt-4 border-t border-[rgb(var(--color-border))]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1">
                        <label className="text-sm font-semibold text-[rgb(var(--color-text))]">Application Name</label>
                        <input
                            type="text"
            ...
            <div className="space-y-6 pt-6 border-t border-[rgb(var(--color-border))]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <ToggleCard
                        ...
                        icon={<Lock size={18} />}
                        onChange={() => handleChange('security', 'twoFA', !config.security.twoFA.admin, 'admin')}
                    />
                    ...
                    <ToggleCard
                        ...
                        icon={<Lock size={18} />}
                        onChange={() => handleChange('security', 'twoFA', !config.security.twoFA.dealer, 'dealer')}
                    />
                </div>
                ...
                <div className="bg-[rgb(var(--color-error))/0.05] border border-[rgb(var(--color-error))/0.2] rounded-lg p-4 flex items-center justify-between">
                    <div className="space-y-1">
                        <div className="flex items-center gap-2">
                            ...
                            <h4 className="font-bold text-[rgb(var(--color-error))]">System Freeze (Maintenance Mode)</h4>
                        </div>
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                        <input
                            ...
                            onChange={() => handleChange('security', 'maintenanceMode', !config.security.maintenanceMode)}
                        />
            ...
            <div className="space-y-6 pt-6 border-t border-[rgb(var(--color-border))]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                        <h4 className="text-xs font-semibold text-[rgb(var(--color-text-muted))]">SMTP SERVER (Mock)</h4>
                        <div className="space-y-3">
                            <input type="text" placeholder="SMTP Host" className="w-full px-4 py-2 text-sm rounded-lg border border-[rgb(var(--color-border))] bg-[rgb(var(--color-background))] text-[rgb(var(--color-text))]" value={config.communication.smtp.host} onChange={(e) => handleChange('communication', 'smtp', e.target.value, 'host')} />
                            <input type="text" placeholder="Port" className="w-full px-4 py-2 text-sm rounded-lg border border-[rgb(var(--color-border))] bg-[rgb(var(--color-background))] text-[rgb(var(--color-text))]" value={config.communication.smtp.port} onChange={(e) => handleChange('communication', 'smtp', e.target.value, 'port')} />
                        </div>
                    </div>
                    <div className="space-y-4">
                        <h4 className="text-xs font-semibold text-[rgb(var(--color-text-muted))]">NOTIFICATIONS</h4>
...
const SwitchRow = ({ label, checked, setter }) => (
    <div className="flex items-center justify-between py-1">
        ...
        <label className="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" className="sr-only peer" checked={checked} onChange={(e) => setter(e.target.checked)} />
