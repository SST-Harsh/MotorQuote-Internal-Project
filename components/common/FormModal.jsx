...
export default function GenericFormModal({
                ...
                <div key={idx} className={field.className || "space-y-2"}>
                    ...
                    <div className="grid grid-cols-2 gap-2 p-3 bg-[rgb(var(--color-background))] border border-[rgb(var(--color-border))] rounded-xl">
                        {field.options?.map(opt => (
                            <label key={opt.value} className="flex items-center gap-2 cursor-pointer hover:bg-[rgb(var(--color-surface))] p-1.5 rounded-lg transition-colors">
                                <input
                                    type="checkbox"
                                    name={field.name}
                                    value={opt.value}
                    ...
                    {error && <p className="text-xs text-[rgb(var(--color-error))] font-medium mt-1">{error}</p>}
                </div>
            );
        }
    ...
    return createPortal(
        <div className="fixed inset-0 h-full z-[9999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className={`bg-[rgb(var(--color-surface))] rounded-2xl w-full ${props.maxWidth || 'max-w-lg'} shadow-2xl border border-[rgb(var(--color-border))] overflow-hidden flex flex-col max-h-[90vh]`}>
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-[rgb(var(--color-border))] bg-[rgb(var(--color-background))]">
                    ...
                    <button onClick={onClose} className="p-2 text-[rgb(var(--color-text))] hover:bg-[rgb(var(--color-surface))] rounded-xl transition-colors">
                        <X size={20} />
                    </button>
                </div>
                            ...
                            return (
                                <div key={idx}>
                                    ...
                                    <div className={field.className || "grid grid-cols-2 gap-4"}>
                                        {field.fields.map((subField, subIdx) => renderField(subField, `${idx}-${subIdx}`))}
                                    </div>
                                </div>
                            );
                        }
