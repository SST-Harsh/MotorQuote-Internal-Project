  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-[rgb(var(--color-surface))] p-4 rounded-2xl border border-[rgb(var(--color-border))] shadow-sm">
        ...
        {extraControls && <div className="flex-shrink-0">{extraControls}</div>}
            ...
            <thead>
              <tr className="bg-[rgb(var(--color-background))/0.5]">
                ...
                {columns.map((col, idx) => (
                  ...
                  >
                    <div
                      className={`flex items-center gap-2 ${col.className?.includes('text-center') || col.className?.includes('justify-center') ? 'justify-center' : ''}`}
                    >
                      {col.header}
                      {col.sortable && (
                        <ArrowUpDown
                          size={14}
                          className={`text-gray-400 opacity-0 group-hover:opacity-100 ${sortConfig.key === (col.sortKey || col.accessor) ? 'opacity-100 text-blue-600' : ''}`}
                        />
                      )}
                    </div>
                  </th>
            ...
            <tbody className="divide-y divide-[rgb(var(--color-border))]">
              {displayedData.length > 0 ? (
                displayedData.map((item) => {
                  ...
                  const isHighlighted =
                    activeHighlightId &&
                    (item.id === activeHighlightId || item.id === `#${activeHighlightId}`);
                  return (
                    <tr
                      key={item.id}
                      id={`row-${item.id}`}
