...
import {
    LayoutDashboard, Building2, FileText, ClipboardCheck,
    Users, UserCog, Settings, Edit2, Bell, MessageSquare, BookOpen
} from "lucide-react";
...
export default function UnifiedDashboardLayout({ children }) {
            ...
            return [
                {
                    ...
                    items: [
                        ...
                        { label: 'Dealerships', icon: <Building2 size={20} />, badge: 12, href: '/dealerships' },
                        { label: 'Quotes', icon: <FileText size={20} />, badge: 58, href: '/quotes' },
                        { label: 'Approvals', icon: <ClipboardCheck size={20} />, badge: 7, href: '/approvals' },
                    ],
                },
                {
                    title: 'Users',
        ...
        if (user.role === 'dealer') {
            return [
                ...
                },
                {
                    title: 'Account',
                    items: [
                        { label: 'Settings', icon: <Users size={20} />, href: '/settings' },
                    ],
                },
            ];
        }
    ...
    const displayUser = useMemo(() => user ? {
        ...
        role: user.role,
        avatar: user.avatar || (user.role === 'dealer' ? 'https://i.pravatar.cc/80?img=47' : 'https://i.pravatar.cc/80?img=33')
    } : null, [user]);
